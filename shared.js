...
// careerOk:true = a plain counting total that can be summed across multiple seasons.
const NFL_GROUPS = {
  passing: {
...
// Pull the numeric value of one stat out of an athlete's category values array.
...
const MAX_PAGES = 6;   // 50/page -> up to 300 players

async function fetchOneSeasonRaw(cat, key, order, season){
  ...
  const pages = Math.min((first.pagination && first.pagination.pages) || 1, MAX_PAGES);
...
// group: key into NFL_GROUPS · statDef: one of that group's stats · season: "2024" | "range:a-b" | "decade:d"
// position: 'ALL' or a position abbreviation to filter the pool.
async function fetchPlayers(group, statDef, season, pool, position){
  ...
  let years = [];
  if(String(season).startsWith('range:')){
  ...
  if(years.length===1){
    const {athletes, namesByCat} = await fetchOneSeasonRaw(cat, statDef.key, order, years[0]);
    const mapped=[];
  ...
  // multi-year: sum counting totals per athlete id
  const totals={};
  const perSeason = await Promise.all(years.map(y => fetchOneSeasonRaw(cat, statDef.key, order, y).catch(()=>({athletes:[],namesByCat:{}}))));
